  ...
  ({int amount, String label}) _resolveDiscount() {
    ...
    if (_chosenPromoId != null) {
      ...
      if (match.isNotEmpty) {
        final p = match.first;
        return (
          amount: DbService.promoDiscountAmount(p, cartSubtotal: _subtotal, productSubtotals: _productSubtotals),
          label: p.name,
        );
      }
    }
            ...
            content: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  ...valid.map((p) {
                    final amt = DbService.promoDiscountAmount(p, cartSubtotal: _subtotal, productSubtotals: _productSubtotals);
