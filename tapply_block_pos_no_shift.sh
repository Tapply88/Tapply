          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const Icon(Icons.lock_clock, size: 40, color: _navy),
                const SizedBox(height: 12),
                const Text('Belum Ada Shift Aktif', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18, color: _navy)),
                const SizedBox(height: 8),
                const Text(
                  'Mulai shift dulu (catat modal awal) sebelum bisa transaksi di kasir.',
                  textAlign: TextAlign.center,
                  style: TextStyle(color: Colors.grey),
                ),
                const SizedBox(height: 20),
                FilledButton(
                  style: FilledButton.styleFrom(backgroundColor: _navy, padding: const EdgeInsets.symmetric(horizontal: 24, vertical: 14)),
                  onPressed: () async {
                    await Navigator.push(context, MaterialPageRoute(builder: (_) => const ShiftScreen()));
                    if (mounted) setState(() {});
                  },
