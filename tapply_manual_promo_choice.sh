...
class _CashierScreenState extends State<CashierScreen> {
  ...
  ({int amount, String label}) _resolveDiscount() {
    final valid = DbService.validPromosFor(_subtotal);

    if (_chosenPromoId == 'NONE') {
      if (DbService.discountEnabled) {
        return (amount: (_subtotal * DbService.discountPercent / 100).round(), label: DbService.discountPromoName);
      }
    ...
    if (_chosenPromoId != null) {
      ...
      if (match.isNotEmpty) {
        final p = match.first;
        return (amount: DbService.promoDiscountAmount(p, _subtotal), label: p.name);
      }
    }
    ...
    if (valid.length > 1) {
      return (amount: 0, label: ''); // nunggu kasir pilih
    }
            ...
            content: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  ...valid.map((p) {
                    final amt = DbService.promoDiscountAmount(p, _subtotal);
                    return RadioListTile<String>(
                      contentPadding: EdgeInsets.zero,
                      title: Text(p.name),
                      subtitle: Text('-${_currency.format(amt)}'),
                      value: p.id,
                      groupValue: temp,
                      onChanged: (v) => setDialogState(() => temp = v),
                    );
            ...
            child: Container(
              ...
              child: Column(
                children: [
                  ...
                  if (DbService.validPromosFor(_subtotal).isNotEmpty) _buildPromoBanner(),
                  const Divider(height: 1),
                  Padding(
                    padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 10),
                    child: Column(
