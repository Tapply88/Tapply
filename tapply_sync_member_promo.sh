...
echo 'Selesai. Untuk app: flutter clean && flutter pub get && flutter run -d web-server --web-port 8080 --release'
echo 'Untuk server/: git add . && git commit -m "sync member+promo" && git push (Railway auto-redeploy)'
