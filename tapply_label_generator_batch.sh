...
class _LabelGeneratorScreenState extends State<LabelGeneratorScreen> {
  ...
  static const _sizes = {
    '60x40mm': Size(220, 150),
    '50x30mm': Size(190, 120),
    '40x30mm': Size(160, 120),
  };
  ...
  Widget _buildLabelCard(Product p, int barcodeNum, Size size) {
    final qrData = 'TAPPLY|${p.sku}|$barcodeNum|${p.productionDate?.toIso8601String() ?? ""}';
    return Container(
      width: size.width,
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (p.productionDate != null)
                      Text('Prod: ${_dateFmt.format(p.productionDate!)}', style: const TextStyle(fontSize: 8.5, color: Colors.black54)),
                    if (p.expiryDate != null)
                      Text('Exp: ${_dateFmt.format(p.expiryDate!)}', style: const TextStyle(fontSize: 8.5, color: Colors.black54)),
                  ],
                ),
              ),
