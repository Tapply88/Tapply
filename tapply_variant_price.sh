cat > lib/models/variation.dart << 'VAREOF'
  ...
  static Future<({bool success, String message})> pullFromCloud() async {
    ...
    try {
      ...
      for (final raw in (data['staff'] as List? ?? [])) {
        final id = raw['id'] as String;
        final existing = staffBoxRef.get(id);
        if (existing != null) {
          existing.name = raw['name'];
          existing.role = raw['role'] ?? existing.role;
          existing.pin = raw['pin'] ?? existing.pin;
          await existing.save();
        } else {
...
class _CashierScreenState extends State<CashierScreen> {
  ...
  Future<void> _openProductModifier(Product p) async {
    ...
    final variationPriceMap = {for (final v in availableVariations) v.name: v.price};
    ...
    final result = await showDialog<bool>(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setDialogState) {
          ...
          int unit = p.price + variationPrice + addonTotal;
                          ...
                          return Expanded(
                            child: Padding(
                              ...
                              child: OutlinedButton(
                                ...
                                child: Text(v.price > 0 ? '${v.name} (+${_currency.format(v.price)})' : v.name),
                              ),
                            ),
                          );
                        }).toList(),
